...
num_label =8000
model_num = 10005
pre=np.zeros((10,num_data),dtype=float)
...
for i in range(model_num,model_num+1):
    f=open(str1+str2+str(i)+str5,"r")
    for j in range(num_data):
        a, b, c, d = map(float, f.readline().split())
        pre[i-model_num][j]=d   
    max = np.max(pre[i-model_num])
    min = np.min(pre[i-model_num])

    for k in range(num_data):
        pre[i-model_num][k]=MaxMinNormalization(pre[i-model_num][k],max,min)
    f.close()

pre=pre[:,np.newaxis,:]
predict=pre[0:10,0:1,0:num_data].reshape(10,1,24,24)

# zoom_factor = 24 / 21 
# dataa_resized = np.zeros((10, 1, 24, 24))  # 初始化插值后的数组
# for i in range(10):
#     dataa_resized[i, 0] = zoom(predict[i, 0], zoom=zoom_factor, order=3)  # order=3表示双三次插值
# predict =  dataa_resized
# print(predict.shape)




# predict=pre[0:10,0:1,0:576].reshape(10,1,24,24)
...
class LightMUNet(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        x = self.convInit(x)   
        # print(x.shape)  #x.shape=[512, 8, 21, 21]
        ...
        for down in self.down_layers:            
            x = down(x)
            # print(f"After downsam!pling in encode, x shape: {x.shape}")  # 添加此行打印语句
            down_x.append(x)
            # print("Shapes stored in down_x:")
            # for i, feature in enumerate(down_x):
            #      
    ...
    def decode(self, x: torch.Tensor, down_x: list[torch.Tensor]) -> torch.Tensor:
        for i, (up, upl) in enumerate(zip(self.up_samples, self.up_layers)):
            x_up = up(x)
            # print(f"Step {i}: up(x).shape = {x_up.shape}, down_x[{i + 1}].shape = {down_x[i + 1].shape}")              
            x = up(x) + down_x[i + 1]            
            x = upl(x)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        batch_size = x.size(0)
        x_flat = x.view(batch_size, -1)  # 展平后与 self.out 维度匹配
        output = self.out(x_flat)        
              
        return output

# unet = LightMUNet().cuda()
unet = LightMUNet().to(device0)
model = torch.load('model/min_loss0.0-LightM-UNet.pth')
model=model.eval()


res=model(torch.tensor(np.expand_dims(predict[0],axis=0),dtype=torch.float32).cuda())
f_w=open("predict/result.txt","w")
for i in range(8000):
    f_w.write(str(res[0][i].data.cpu().numpy()))
    f_w.write("\n")
f_w.close()